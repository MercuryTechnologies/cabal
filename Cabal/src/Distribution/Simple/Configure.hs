    ...
    | exact_config =
        ...
        if isInternalDep && not use_external_internal_deps
          ...
          -- those are just True.
            ...
            if sublibs == mainLibSet
              then
                let depComponentName =
                      CLibName $ LSubLibName $ packageNameToUnqualComponentName depName
                 in if Map.member (pn, depComponentName) requiredDepsMap
                      then Satisfied
                      else Unsatisfied $ MissingComponent depName
              else
                let invisibleLibraries = NES.filter (not . visible) sublibs
                 in case nonEmpty $ Set.toList invisibleLibraries of
                      Nothing -> Satisfied
